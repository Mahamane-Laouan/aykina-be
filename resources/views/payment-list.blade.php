...
@section('content')
    ...
    <div class="row align-items-center">
        <div class="col-md-3" style="padding-top: 18px;">
            <div class="mb-3">
                <h5 class="card-title">Transactions <span class="text-muted fw-normal ms-2">({{ $records->total() }})</span>
                </h5>
                <p class="text-muted">Order Management / Transactions</p>
            </div>
        </div>
                        ...
                        <table class="table table-nowrap align-middle">
                            <thead class="table-light">
                                <tr>
                                    ...
                                    <th scope="col" style="color: #ffff;">Total Paid Amount</th>
                                </tr>
                            </thead>
                                        ...
                                        <td colspan="6" class="text-center">
                                            <div
                                                ...
                                                <div
                                                    ...
                                                    <i class="bx bx-wallet" style="font-size: 2.5rem; color: #fff;"></i>
                                                </div>
                                                <p
        ...
        <div style="display: flex; flex-direction: column; width: fit-content;">
            ...
            @if ($user->user)
                <div class="text-body" style="display: flex; align-items: center;">
                    <a href="{{ route('user-view', $user->user_id) }}" style="color: #246FC1;">
                        {{ $user->user->firstname }}
            ...
            @endif
                                            ...
                                            <td>
                                                @if ($user->payment_mode === 'paypal')
                                                    <span class="badge bg-info-subtle text-info font-size-15">PayPal</span>
                                                @elseif ($user->payment_mode === 'wallet')
                                                    <span
                                                        class="badge bg-success-subtle text-success font-size-15">Wallet</span>
                                                @elseif ($user->payment_mode === 'google pay')
                                                    <span class="badge bg-warning-subtle text-warning font-size-15">Google
                                                        Pay</span>
                                                @elseif ($user->payment_mode === 'stripe')
                                                    <span
                                                        class="badge bg-primary-subtle text-primary font-size-15">Stripe</span>
                                                @elseif ($user->payment_mode === 'Razor Pay')
                                                    <span class="badge bg-danger-subtle text-danger font-size-15">Razor
                                                        Pay</span>
                                                @elseif ($user->payment_mode === 'Flutter Wave')
                                                    <span
                                                        class="badge bg-secondary-subtle text-secondary font-size-15">Flutter
                                                        Wave</span>
                                                @elseif ($user->payment_mode === 'apple pay')
                                                    <span class="badge bg-dark-subtle text-dark font-size-15">Apple
                                                        Pay</span>
                                                @else
                                                    <span
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function getPaymentModeBadge(paymentMode) {
            switch (paymentMode.toLowerCase()) {
                case 'paypal':
                    return '<span class="badge bg-info-subtle text-info font-size-15">PayPal</span>';
                case 'wallet':
                    return '<span class="badge bg-success-subtle text-success font-size-15">Wallet</span>';
                case 'google pay':
                    return '<span class="badge bg-warning-subtle text-warning font-size-15">Google Pay</span>';
                case 'stripe':
                    return '<span class="badge bg-primary-subtle text-primary font-size-15">Stripe</span>';
                case 'razor pay':
                    return '<span class="badge bg-danger-subtle text-danger font-size-15">Razor Pay</span>';
                case 'flutter wave':
                    return '<span class="badge bg-secondary-subtle text-secondary font-size-15">Flutter Wave</span>';
                case 'apple pay':
                    return '<span class="badge bg-dark-subtle text-dark font-size-15">Apple Pay</span>';
                default:
                    return '<span class="badge bg-secondary-subtle text-secondary font-size-15"></span>';
            }
        }
        ...
        if (searchInput) {
            searchInput.addEventListener('keyup', function() {
                ...
                fetch(`{{ route('payment-list') }}?search=${query}`, {
                        headers: {
                    ...
                    .then(data => {
                        ...
                        if (data.records.length > 0) {
                            data.records.forEach(record => {
                                ...
                                const paymentStatus = record.p_status === 1 ?
                                    '<span class="badge bg-success-subtle text-success font-size-15">Paid</span>' :
                                    '<span class="badge bg-danger-subtle text-danger font-size-15">Unpaid</span>';
                                ...
                                <tr>
                                    <td>
                                        <a href="${record.view_url}" style="color: #246FC1;">
                                            #${record.id}
                                        </a>
                                    </td>
                                    <td>
