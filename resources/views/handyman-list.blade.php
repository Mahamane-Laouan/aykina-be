...
@section('content')
    <div class="row align-items-center">
        <div class="col-md-3" style="padding-top: 18px;">
            <div class="mb-3">
                <h5 class="card-title">Handyman List<span class="text-muted fw-normal ms-2">({{ $users->total() }})</span>
                </h5>
                <p class="text-muted">Handyman / Handyman List</p>
            </div>
        </div>
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="d-flex justify-content-between align-items-center" style="padding-top: 15px;">
                    <!-- Title -->
                    <h2 class="card-title" style="margin-left: 25px;">Handyman List</h2>

                    <!-- Add Faq Button -->
                    <a href="{{ route('handyman-add') }}" class="btn btn-primary" style="margin-right: 25px;">Add
                        Handyman</a>
                </div>
                <div class="card-body">
                                                        ...
                                                        @if ($user->provider)
                                                            <div class="text-body">
                                                                <a href="{{ route('provider-view', $user->provider_id) }}"
                                                                    style="color: #246FC1; text-decoration: none; display: flex; align-items: center">
                                                                    {{ $user->provider->firstname }}
                                                                    {{ Str::limit($user->provider->lastname ?? '', 15) }}
                                            ...
                                            <td>
                                                <div class="form-check form-switch mb-3" dir="ltr"
                                                    ...
                                                    <input type="checkbox" class="form-check-input"
                                                        ...
                                                        onchange="ChangeHandymanListBlocked({{ $user->id }}, this.checked)"
                                                ...
                                                <ul class="list-inline mb-0">
                                                    <li class="list-inline-item">
                                                        <a href="{{ route('handyman-view', $user->id) }}"
                                                            ...
                                                            title="View" class="px-2 text-secondary"><i
                                                                class="bx bx-show font-size-18"></i></a>
                                                    </li>
                                                    <li class="list-inline-item">
                                                        <a href="{{ route('handyman-edit', $user->id) }}"
                                                            data-bs-toggle="tooltip" data-bs-placement="top"
                                                    ...
                                                    <li class="list-inline-item">
                                                        <a data-bs-toggle="tooltip" data-bs-placement="top"
                                                            ...
                                                            onclick="deleteHandyman({{ $user->id }})"><i
...
@section('scripts')
    ...
    <script>
        ...
        function ChangeHandymanListBlocked(id, isChecked) {
            ...
            $.ajax({
                url: baseUrl + 'change-handymanlistblocked/' + id,
                type: 'get',
                headers: {
...
<script>
    var baseUrl = "{{ url('/') }}/";
    // Delete Provider
    function deleteHandyman(userId) {
        Swal.fire({
            title: 'Are you sure?',
            text: "You won't be able to revert Handyman!",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonText: 'Yes, Delete Handyman!',
            customClass: {
...
<script>
    ...
    const commission = "{{ $commission }}"; // Fetch this from the controller
</script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (searchInput) {
            searchInput.addEventListener('keyup', function() {
                ...
                fetch(`{{ route('handyman-list') }}?search=${query}`, {
                        headers: {
    ...
    <td>
        <div class="form-check form-switch mb-3" dir="ltr" style="padding-top: 10px;">
            <input type="checkbox" class="form-check-input"
                ...
                onchange="ChangeHandymanListBlocked(${record.id}, this.checked)" style="transform: scale(1.1);">
...
`;
    ...
    <div style="display: flex; align-items: center;">
        ...
        <div style="display: flex; flex-direction: column; width: fit-content;">
            <div style="display: flex; align-items: center;">
                <a href="${record.view_url}" class="text-body text-decoration-none me-1">
                    ${highlightedName}
                </a>
    ...
    <div style="display: flex; align-items: center;">
        <div>
            ${record.provider && record.provider.profile_pic
            ...
            <div style="display: flex; align-items: center;">
                ${record.provider
                    ? `<a href="${record.provider.profile_url}" style="color: #246FC1; margin-right: 5px;">
                        ${record.provider.firstname} ${truncateText(record.provider.lastname ?? '', 15)}
                    </a>
                    <i class="bx bxs-star font-size-14 text-warning"></i>
                    <span class="font-size-14">${record.provider.avg_provider_review ?? '0'}</span>`
                    : '<small></small>'}
            </div>
            <small class="text-muted">${truncateText(record.provider ? record.provider.email : '', 15)}</small>
        </div>
    </div>
</td>
                                ...
                                <td>
                                    <ul class="list-inline mb-0">
                                        ...
                                        <li class="list-inline-item">
                                            <a href="${record.edit_url}" class="px-2 text-primary" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit">
                                                <i class="bx bx-pencil font-size-18"></i>
                                            </a>
                                        </li>
                                        <li class="list-inline-item">
                                            <a class="px-2 text-danger" onclick="deleteHandyman(${record.id})" data-bs-toggle="tooltip" data-bs-placement="top" title="Delete">
