...
@section('content')
    ...
    <div class="row align-items-center">
        <div class="col-md-3" style="padding-top: 18px;">
            <div class="mb-3">
                <h5 class="card-title">Provider Ratings List <span
                        class="text-muted fw-normal ms-2">({{ $records->total() }})</span>
                </h5>
                <p class="text-muted">Ratings / Provider Ratings List</p>
            </div>
        </div>
    ...
    @if ($user->provider)
        <div class="text-body d-flex align-items-center">
            <a href="{{ route('provider-view', $user->provider->id) }}" style="color: #246FC1;">
    ...
    @endif
                                            ...
                                            <td>
                                                <a href="{{ route('service-edit', $user->service_id) }}"
                                                    style="color: #246FC1;">
                                                    {{ $user->service ? Str::limit($user->service->service_name, 20) : '' }}
                                                </a>
                                            </td>
                                                ...
                                                <ul class="list-inline mb-0">
                                                    <li class="list-inline-item">
                                                        <a data-bs-toggle="tooltip" data-bs-placement="top" title="Delete"
                                                            ...
                                                            onclick="deleteProviderReview({{ $user->id }})"><i
...
<script>
    ...
    function deleteProviderReview(id) {
        Swal.fire({
            title: 'Are you sure?',
            ...
            if (result.value) {
                $.ajax({
                    url: baseUrl + 'providerreview-delete/' + id,
                    type: 'POST',
                    headers: {
                    ...
                    success: function() {
                        Swal.fire({
                            icon: 'success',
                            title: 'Deleted!',
                            text: 'Provider Review has been removed.',
                            customClass: {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (searchInput) {
            searchInput.addEventListener('keyup', function() {
                    ...
                    fetch(`{{ route('provider-reviewlist') }}?search=${query}`, {
                            headers: {
                                    ...
                                    <td>
                                        <ul class="list-inline mb-0">
                                            <li class="list-inline-item">
                                                <a data-bs-toggle="tooltip" data-bs-placement="top" title="Delete"
                                                    class="px-2 text-danger"
                                                    onclick="deleteProviderReview(${record.id})"><i
                                                    class="bx bx-trash-alt font-size-18"></i></a>
                                            </li>
                                        </ul>
                                    </td>
