...
@section('content')
    ...
    <div class="row align-items-center">
        <div class="col-md-3" style="padding-top: 18px;">
            <div class="mb-3">
                <h5 class="card-title">Handyman Ratings List <span
                        class="text-muted fw-normal ms-2">({{ $records->total() }})</span>
                </h5>
                <p class="text-muted">Handyman / Handyman Ratings List</p>
            </div>
        </div>
    ...
    @if ($user->handyman)
        <div class="text-body d-flex align-items-center">
            <a href="{{ route('providerhandyman-view', $user->handyman->id) }}" style="color: #246FC1;">
                {{ $user->handyman->firstname }}
                {{ Str::limit($user->handyman->lastname ?? '', 20) }}
            </a>
            ...
            <span class="ms-1 d-flex align-items-center" style="color: #000000;">
                ...
                <span class="font-size-14">{{ $user->handyman->avg_handyman_review ?? '0' }}</span>
            </span>
        </div>
        <small class="text-muted">
            {{ Str::limit($user->handyman->email ?? '', 30) }}
        </small>
    @else
    ...
    @endif
                                            ...
                                            <td style="text-wrap: auto;">
                                                <div>
                                                    @for ($i = 1; $i <= 5; $i++)
                                                        <i
                                                            class="{{ $i <= $user['star_count'] ? 'fas' : 'far' }} fa-star text-warning"></i>
                                                    @endfor
                                                    <div>{{ $user['text'] }}</div>
                                                </div>
                                            </td>
                                                ...
                                                <ul class="list-inline mb-0">
                                                    <li class="list-inline-item">
                                                        <a data-bs-toggle="tooltip" data-bs-placement="top" title="Delete"
                                                            ...
                                                            onclick="deleteProviderHandymanReview({{ $user->id }})"><i
...
<script>
    ...
    function deleteProviderHandymanReview(id) {
        Swal.fire({
            title: 'Are you sure?',
            ...
            if (result.value) {
                $.ajax({
                    url: baseUrl + 'providerhandymanreview-delete/' + id,
                    type: 'POST',
                    headers: {
                    ...
                    success: function() {
                        Swal.fire({
                            icon: 'success',
                            title: 'Deleted!',
                            text: 'Handyman Review has been removed.',
                            customClass: {
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
                ...
                if (searchInput) {
                    searchInput.addEventListener('keyup', function() {
                                ...
                                fetch(`{{ route('providerhandyman-reviewlist') }}?search=${query}`, {
                                        headers: {
                                    ...
                                    .then(data => {
                                            ...
                                            if (data.records.length > 0) {
                                                data.records.forEach(record => {
                                                            const userProfile = record.handyman.profile_pic ?
                                                                `<img src="${record.handyman.profile_pic}" alt="User Image" class="avatar rounded-circle img-thumbnail me-2" style="min-width: 40px; height: 40px; object-fit: cover;">` :
    ...
    <div style="display: flex; align-items: center;">
        ...
        <div style="display: flex; flex-direction: column; width: fit-content; margin-left: 8px;">
            <div style="display: flex; align-items: center;">
                <a href="${record.handyman.profile_url}" style="color: #246FC1;">
                    ${record.handyman.firstname} ${truncateText(record.handyman.lastname ?? '', 20)}
                </a>
                <span style="display: flex; align-items: center; margin-left: 5px; color: #000000;">
                    <i class="bx bxs-star font-size-14 text-warning"></i>
                    <span class="font-size-14" style="margin-left: 2px;">
                        ${record.handyman.avg_handyman_review ?? '0'}
                    </span>
                </span>
            </div>
            <small class="text-muted">${truncateText(record.handyman.email ?? '', 30)}</small>
        </div>
    </div>
</td>
                                    ...
                                    <td>
                                        <div>
                                            ${Array.from({ length: 5 }, (_, i) => `
                                                <i class="${i < record.star_count ? 'fas' : 'far'} fa-star text-warning"></i>`
                                            ).join('')}
                                            <div>${record.text}</div>
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        <ul class="list-inline mb-0">
                                            <li class="list-inline-item">
                                                <a data-bs-toggle="tooltip" data-bs-placement="top" title="Delete"
                                                    class="px-2 text-danger"
                                                    onclick="deleteProviderHandymanReview(${record.id})"><i
                                                    class="bx bx-trash-alt font-size-18"></i></a>
                                            </li>
                                        </ul>
                                    </td>
