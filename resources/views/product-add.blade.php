                                                ...
                                                <select class="form-control" data-trigger id="vid" name="vid">
                                                    ...
                                                    @foreach ($vendors as $value)
                                                        <option value="{{ $value->id }}"
                                                            {{ old('vid') == $value->id ? 'selected' : '' }}>
                                                            {{ $value->firstname }} {{ $value->lastname }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                                <span class="text-danger">{{ $errors->first('vid') }}</span>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-md-4">
                                            <div class="mb-3">
                                                <label for="sub_category_id" class="form-label"
                                                    style="font-size: 16px; font-weight: bold;">Services</label>
                                                <select id="sub_category_id" name="service_id[]" class="form-select select2"
                                                    data-placeholder="Select Services" multiple
                                                    style="padding: 12px; font-size: 16px; border: 1px solid #ddd; border-radius: 5px; background-color: #f9f9f9; box-shadow: 0 2px 5px rgba(0, 0, 0, 0.1); transition: all 0.3s ease-in-out;">
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                $('#sub_category_id').select2({
                    placeholder: "Select Services", // Add placeholder text
                    width: '100%' // Make the dropdown 100% width
                });

                // On provider change, fetch services dynamically
                document.getElementById('vid').addEventListener('change', function() {
                    var providerId = this.value;
                    console.log('Selected Provider ID:', providerId); // Debugging line

                    // Clear the error message before new selection
                    document.getElementById('sub_category_error').textContent = '';

                    if (providerId) {
                        // Make an Ajax request to fetch services based on the selected provider
                        fetch(`/get-services/${providerId}`) // Ensure this endpoint matches your route
                            .then(response => response.json())
                            .then(data => {
                                console.log('Fetched Services:', data); // Debugging line

                                // Clear the current service options
                                const serviceSelect = document.getElementById('sub_category_id');
                                serviceSelect.innerHTML =
                                    '<option value="">Select Services</option>'; // Reset dropdown

                                // Check if data is not empty and contains the expected structure
                                if (Array.isArray(data) && data.length > 0) {
                                    // Populate the service dropdown with the received data
                                    data.forEach(service => {
                                        const option = document.createElement('option');
                                        option.value = service.id; // Ensure correct property
                                        option.textContent = service
                                            .service_name; // Ensure correct property
                                        serviceSelect.appendChild(option);
                                    });
                                } else {
                                    // Handle case when no services are returned
                                    const option = document.createElement('option');
                                    option.value = '';
                        ...
                        // Clear the service options if no provider is selected
                        const serviceSelect = document.getElementById('sub_category_id');
