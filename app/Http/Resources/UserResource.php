...
class UserResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        if ($user_role == "Handyman" || $user_role == "handyman") {

            $commission = Commissions::where('id', "1")->first();
            $commission_value = $commission->value;
        }

        if ($user_role == "provider" || $user_role == "Provider") {

            $commission = Commissions::where('id', "2")->first();
            $commission_value = $commission->value;

            $commission_al = Commissions::where('id', "3")->first();
            $commission_value_done = $commission_al->value;
        }

        // if ($user_role == "provider" || $user_role == "Provider") {

        //     $commission = Commissions::where('id', "3")->first();
        //     $commission_value_done = $commission->value;
        // }

        $profile_pic = "";

        $provider_details = User::where('id', $this->id)->first();
        ...
        if ($pro_seen == "1") {
            ...
            if (!empty($this->profile_pic)) {
                ...
                $profile_pic =  url('/images/user/' . $this->profile_pic);
            } else {
        ...
        if ($pro_seen == "1") {
            ...
            $total_reviews_count = ServiceReview::where('provider_id', $this->id)->count();
            $total_average_count = ServiceReview::where('provider_id', $this->id)->sum('star_count');
            ...
            if ($total_reviews_count) {
                $total_reviews_sum = $total_reviews_count > 0 ? $total_average_count / $total_reviews_count : 0.0;
            } else {
                $total_reviews_sum = 0.0;
            }
        }
        ...
        return [
            ...
            // 'commision' => "20%",
            'commision' =>  $commission_value ? (string)$commission_value . "%" : "",
            'product_commision' =>  $commission_value_done ? (string)$commission_value_done . "%" : "",
            'country_flag' => $this->country_flag ?? "",
            'people_id' => (string)$this->people_id ?? "",
            ...
            'provider_avg_rating' => $total_reviews_sum ? number_format($total_reviews_sum, 1) : "0.0",
            'provider_total_rating' => (string)$total_reviews_count ?? "",
