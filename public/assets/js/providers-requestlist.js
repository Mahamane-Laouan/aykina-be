...
$(function () {
  ...
  if (dt_user_table.length) {
    var dt_user = dt_user_table.DataTable({
      ...
      processing: true,
      pagination: 10,
      ajax: baseUrl + 'getproviderrequestdata',
      columns: [
        { data: 'id' },
        { data: 'firstname' },
        { data: 'created_at' },
      ...
      columnDefs: [
        ...
        },
        {
          ...
          targets: 4,
          render: function (data, type, full, meta) {
            var $confirmation = full['confirmation'];

            if ($confirmation == '1') {
              return '<span class="badge bg-label-success" text-capitalized>' + 'Approved' + '</span>';
            } else if ($confirmation == '0') {
              return '<span class="badge bg-label-danger" text-capitalized>' + 'Deny' + '</span>';
            } else {
              return '<span class="badge bg-label-default" text-capitalized>' + $confirmation + '</span>';
            }
          }
        },
        {
          ...
          render: function (data, type, full, meta) {
            ...
            return (
              ...
              '<button type="button" class="btn btn-sm btn-icon" data-bs-toggle="modal" data-bs-target="#editStatusModal' +
              userId +
              '"><i class="bx bx-edit"></i></button>' +
              '<button class="btn btn-sm btn-icon delete-record" onclick="deleteProvider(' +
              userId +
...
function saveUserStatus(userId) {
  var selectedStatus = $('#confirmation' + userId).val();
  var csrfToken = $('meta[name="csrf-token"]').attr('content');

  $.ajax({
    type: 'POST',
    url: '/providers-requestupdate/' + userId,
    headers: {
    ...
    success: function (response) {
      if (response.success) {
        // Hide the modal
        $('#editStatusModal' + userId).modal('hide');
        location.reload();
      }
    },
