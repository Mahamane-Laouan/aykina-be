...
@section('content')
    ...
    <div class="row align-items-center">
        <div class="col-md-3" style="padding-top: 18px;">
            <div class="mb-3">
                <h5 class="card-title">Sub Category List <span
                        class="text-muted fw-normal ms-2">({{ $records->total() }})</span>
                </h5>
                <p class="text-muted">Service Management / Sub Category List</p>
            </div>
        </div>
        ...
        <div class="col-md-3" style="padding-top: 18px;">
            <div class="input-group"
                ...
                <input type="text" id="searchSubCategoryInput" class="form-control"
                    placeholder="Search by sub category or category name" aria-label="Search" aria-describedby="search-icon"
        ...
        <div class="col-lg-12">
            <div class="card">
                ...
                <div class="d-flex justify-content-between align-items-center" style="padding-top: 15px;">
                    <h2 class="card-title" style="margin-left: 25px;">Sub Category List</h2>

                    <a href="{{ route('subcategory-add') }}" class="btn btn-primary" style="margin-right: 25px;">Add
                        Sub Category</a>
                </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-nowrap align-middle">
                            <thead class="table-light">
                                <tr>
                                    <th scope="col" style="min-width: 16rem; color: #ffff;">Sub Category</th>
                                    <th scope="col" style="min-width: 16rem; color: #ffff;">Parent Category</th>
                                        ...
                                        <td colspan="4" class="text-center">
                                            <div
                                                ...
                                                <div
                                                    ...
                                                    <i class="bx bx-list-ul" style="font-size: 2.5rem; color: #fff;"></i>
                                                </div>
                                                <p
                                                    style="margin-top: 0.6rem; font-size: 1.1rem; font-weight: 500; color: #444;">
                                                    No Sub Category Found</p>
                                            </div>
                                        </td>
                                    </tr>
                                @else
                                    @foreach ($records as $user)
                                        <tr>
                                            <td>
                                                <a href="{{ route('subcategory-edit', $user->id) }}"
                                                    class="text-body text-decoration-none">
                                                    @if (!empty($user->img))
                                                        <img src="{{ asset('images/subcategory_icon/' . $user->img) }}"
                                                            alt="Product Image" class="avatar rounded-circle img-thumbnail"
                                                            style="min-width: 50px; height: 50px; object-fit: cover;">
                                                    @else
                                                        {{-- Default Avatar --}}
                                                        <div class="avatar rounded-circle img-thumbnail"
                                                            style="background-color: #246FC1; color: #ffff; display: inline-flex; align-items: center; justify-content: center; min-width: 50px; height: 50px;">
                                                            <span style="font-size: 1.2rem;">
                                                                {{ strtoupper(substr($user->c_name ?? '', 0, 1)) }}
                                                            </span>
                                                        </div>
                                                    @endif
                                                    <span style="color: #246FC1;">{{ $user->c_name ?? '' }}</span>
                                                </a>
                                            </td>

                                            <td>
                                                <a href="{{ route('category-edit', $user->category->id) }}"
                                                    style="color: #246FC1; text-decoration: none;">
                                                    {{ $user->category->c_name ?? '' }}
                                                </a>
                                            </td>
                                            ...
                                            <td>
                                                <div class="form-check form-switch mb-3" dir="ltr"
                                                    ...
                                                    <input type="checkbox" class="form-check-input"
                                                        ...
                                                        onchange="changeSubCategoryStatus ({{ $user->id }}, this.checked)"
                                                ...
                                                <ul class="list-inline mb-0">
                                                    ...
                                                    <li class="list-inline-item">
                                                        <a data-bs-toggle="tooltip" data-bs-placement="top" title="Delete"
                                                            ...
                                                            onclick="deleteSubcategory({{ $user->id }})"><i
...
@section('scripts')
    ...
    <script>
        ...
        function changeSubCategoryStatus(id, isChecked) {
            ...
            var statusMessage = isChecked ? "\u2714 SubCategory Activated" : "\u26A0 SubCategory Deactivated";
            ...
            $.ajax({
                url: baseUrl + 'change-subcategorystatus/' + id,
                type: 'get',
                headers: {
...
<script>
    ...
    function deleteSubcategory(id) {
        Swal.fire({
            title: 'Are you sure?',
            text: "You won't be able to revert sub category!",
            icon: 'warning',
            ...
            if (result.value) {
                $.ajax({
                    url: baseUrl + 'subcategory-delete/' + id,
                    type: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    success: function() {
                        Swal.fire({
                            icon: 'success',
                            title: 'Deleted!',
                            text: 'Sub Category has been removed.',
                            customClass: {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (searchInput) {
            searchInput.addEventListener('keyup', function() {
                ...
                fetch(`{{ route('subcategory-list') }}?search=${query}`, {
                        headers: {
    ...
    <td>
        <div class="form-check form-switch mb-3" dir="ltr" style="padding-top: 10px;">
            <input type="checkbox" class="form-check-input"
                ...
                onchange="changeSubCategoryStatus(${record.id}, this.checked)" style="transform: scale(1.1);">
...
const imageUrl = record.img 
    ? `<img src="${record.img}" alt="Product Image" class="avatar rounded-circle img-thumbnail" style="min-width: 50px; height: 50px; object-fit: cover;">`
    : `<div class="avatar rounded-circle img-thumbnail" style="background-color: #246FC1; color: #ffff; display: inline-flex; align-items: center; justify-content: center; min-width: 50px; height: 50px;">
          <span style="font-size: 1.2rem;">${record.c_name ? record.c_name.charAt(0).toUpperCase() : ''}</span>
      </div>`;
            ...
            <a href="${record.edit_url}" class="text-body text-decoration-none">
                ${imageUrl}
                <span style="color: #246FC1;">${record.c_name ?? ''}</span>
            </a>
        </td>
                                     <td>
                                        <a href="${record.category_url}" style="color: #246FC1;">
                                    ...
                                    <td>
                                        <ul class="list-inline mb-0">
                                            ...
                                            <li class="list-inline-item">
                                                <a class="px-2 text-danger" onclick="deleteSubcategory(${record.id})"><i class="bx bx-trash-alt font-size-18"></i></a>
                                            </li>
                                        </ul>
                                    </td>
                                </tr>`;
