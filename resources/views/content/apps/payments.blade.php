                ...
                <div class="tab-pane fade show active" id="store_details" role="tabpanel">
                    <div class="card mb-4">
                        <div class="card-header">
                            <h5 class="card-title m-0">Stripe Payment</h5>
                        </div>
                        <div class="card-body">
                            <form id="addPayment" method="post" action="{{ route('payments-save') }}">
                                @csrf
                                <!-- Payment Stripe Key -->
                                <div class="row mb-3 g-3">
                                    <div class="col-12 col-md-12">
                                        <label class="form-label" for="publish_key">Payment Stripe Key<span
                                                style="color: red;"> *</span></label>
                                        <input type="text"
                                            value="{{ old('publish_key', optional($payments->firstWhere('payment_mode', 'stripe'))->publish_key) }}"
                                            placeholder="Enter Payment Stripe Key" name="publish_key"
                                    ...
                                    <!-- Stripe Secret -->
                                    <div class="col-12 col-md-12">
                                        <label class="form-label" for="secret_key">Stripe Secret<span style="color: red;">
                                                *</span></label>
                                        <input type="text"
                                            value="{{ old('secret_key', optional($payments->firstWhere('payment_mode', 'stripe'))->secret_key) }}"
                                            placeholder="Enter Stripe Secret" name="secret_key"
                                        ...
                                        <div class="w-25 d-flex justify-content-end">
                                            <label class="switch switch-primary switch-sm me-4 pe-2">
                                                <input type="checkbox" name="status" class="switch-input"
                                                    {{ optional($payments->firstWhere('payment_mode', 'stripe'))->status ? 'checked' : '' }}>
