    ...
    <div class="col-lg-12">
        <div class="card">
            ...
            <div class="d-flex justify-content-between align-items-center" style="padding-top: 15px;">
                ...
                <a href="{{ route('service-add') }}" class="btn btn-primary" style="margin-right: 25px;">Add
                    Service</a>
            </div>
                                            ...
                                            <div style="display: flex; align-items: center; gap: 1rem;">
                                                @if ($user->vendor && $user->vendor->profile_pic)
                                                    <img src="{{ asset('images/user/' . $user->vendor->profile_pic) }}"
                                                        alt="Vendor Image" class="avatar rounded-circle img-thumbnail"
                                                        style="min-width: 50px; height: 50px; object-fit: cover;">
                                                @else
                                                    {{-- Default User Image --}}
      ...
      <td>
    @if ($user->category)
        <a href="{{ route('category-edit', $user->category->id) }}" style="color: #246FC1; text-decoration: none;">
            {{ Str::limit($user->category->c_name, 20) }}
        </a>
    @else
                                        ...
                                        <td>
                                            <div class="form-check form-switch mb-3" dir="ltr"
                                                ...
                                                <input type="checkbox" class="form-check-input"
                                                    ...
                                                    onchange="ChangeServiceStatus({{ $user->id }}, this.checked)"
                                        ...
                                        <td>
                                            <div class="form-check form-switch mb-3" dir="ltr"
                                                ...
                                                <input type="checkbox" class="form-check-input"
                                                    ...
                                                    onchange="changeServiceLinkStatus ({{ $user->id }}, this.checked)"
                                                    style="transform: scale(1.1);">
                                            ...
                                            <ul class="list-inline mb-0">
                                                ...
                                                <li class="list-inline-item">
                                                    <a data-bs-toggle="tooltip" data-bs-placement="top"
                                                        ...
                                                        onclick="deleteService({{ $user->id }})"><i
...
<script>
    ...
    function changeServiceLinkStatus(id, isChecked) {
        ...
        $.ajax({
            url: baseUrl + 'change-servicelinkstatus/' + id,
            type: 'get',
            headers: {
...
<script>
    ...
    // ChangeServiceStatus;
    function ChangeServiceStatus(id, type) {
        Swal.fire({
            title: 'Are you sure?',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: baseUrl + 'change-servicestatus/' + id,
                    type: 'get',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (searchInput) {
            searchInput.addEventListener('keyup', function() {
                ...
                fetch(`{{ route('service-list') }}?search=${encodeURIComponent(query)}`, {
                        headers: {
                    ...
                    .then(data => {
                        ...
                        if (data.records.length > 0) {
                            ...
                            data.records.forEach(record => {
                                ...
                                const vendorProfile = record.vendor.profile_pic ?
                                    `<img src="${record.vendor.profile_pic}" alt="Vendor Image" class="avatar rounded-circle img-thumbnail" style="min-width: 50px; height: 50px; object-fit: cover;">` :
                                    `
    ...
    <td>
        <div class="form-check form-switch mb-3" dir="ltr" style="padding-top: 10px;">
            <input type="checkbox" class="form-check-input"
                ...
                onchange="changeServiceLinkStatus(${record.id}, this.checked)" style="transform: scale(1.1);">
...
`;
            ...
            return formattedDuration;
                                    ...
                                    <td>
                                        <div style="display: flex; align-items: center; gap: 1rem;">
                                            ${vendorProfile}
                                            <div>
                                                <a href="${record.vendor.profile_url}" style="color: #246FC1; text-decoration: none;">
                                                    ${record.vendor.firstname} ${record.vendor.lastname}
                                                </a>
                                                <i class="bx bxs-star font-size-14 text-warning"></i>
                    <span class="font-size-14">${record.vendor.avg_provider_review ?? '0'}</span>
                                                <small class="text-muted d-block">${record.vendor.email}</small>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <a href="${record.category_url}" style="color: #246FC1;">
                                            ${record.c_name.length > 20 ? record.c_name.substring(0, 20) + '...' : record.c_name}
                                        </a>
                                    </td>
    ...
    <div class="form-check form-switch" style="padding-top: 2px;">
        <input type="checkbox" class="form-check-input" id="customSwitch${record.id}" ${record.is_features == 1 ? 'checked' : ''} onchange="ChangeServiceStatus(${record.id}, this.checked)" style="transform: scale(1.1);">
        <label class="form-check-label" for="customSwitch${record.id}"></label>
    </div>
</td>
                                    ...
                                    <td>
                                        <ul class="list-inline mb-0">
                                            ...
                                            <li class="list-inline-item">
                                                <a href="#" class="px-2 text-danger" onclick="deleteService(${record.id})" data-bs-toggle="tooltip" title="Delete"><i class="bx bx-trash-alt font-size-18"></i></a>
                                            </li>
                                        </ul>
                                    </td>
                                </tr>`;
    ...
    <td colspan="7" class="text-center">
        <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 1rem;">
            <div style="background: linear-gradient(135deg, #246FC1, #1E5A97); padding: 0.8rem; border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.2);">
                <i class="bx bx-briefcase" style="font-size: 2.5rem; color: #fff;"></i>
            </div>
            <p style="margin-top: 0.6rem; font-size: 1.1rem; font-weight: 500; color: #444;">No Services Available</p>
        </div>
    </td>
</tr>
`;
